...
if (
    ...
    if (!$not_valid_answer && !$_SESSION['same_response']) {
        ...
        $params_feedback = [':fcs_id' => $fcs_id, ':feedback_id' => $guid, ':ip' => $ip];
        ...
        http.onreadystatechange = () => {
            ...
            if (http.readyState == 4 && http.status == 200) {
                ...
                if (http.responseText.length > 0) {
                    ...
                    //console.log(res);
                    for (cnt = 0; cnt < res.length; cnt++) {
                        $('#subject').append($('<option>').attr("value", res[cnt].subject_id).text(res[cnt]
                            .subject_name));
                    }
                }
    ...
    function getFaculty() {
        ...
        http.onreadystatechange = () => {
            if (http.readyState == 4 && http.status == 200) {
                ...
                for (cnt = 0; cnt < res.length; cnt++) {
                    //console.log(res[cnt]);
                    $('#faculty').append($('<option>').attr("value", res[cnt].faculty_id).text(res[cnt]
                        .faculty_name));
                }
            }
        }
